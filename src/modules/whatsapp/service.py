...
class WhatsAppService:
    def __init__(self, meta_client: MetaAPIClient) -> None:
        self.meta_client = meta_client

    async def handle_message(self, message: WebhookMessage, redis: Redis, settings: Settings) -> None:
        conversation_id: UUID | None = None
        try:
            async with AsyncSessionLocal() as db:
                async with db.begin():
                    number_hash = hashlib.sha256(f'{message.from_}{settings.hash_salt}'.encode()).hexdigest()
                    if not await self._allow(redis, number_hash):
                        return
                    conv = await self._conversation(db, number_hash)
    ...
    async def _allow(self, redis: Redis, number_hash: str) -> bool:
    ...
    async def _conversation(self, db: AsyncSession, number_hash: str) -> Conversation:

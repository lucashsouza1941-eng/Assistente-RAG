...
@router.post('/webhook', response_class=PlainTextResponse, status_code=status.HTTP_200_OK, dependencies=[Depends(validate_webhook_signature)])
async def webhook(request: Request, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db_session), redis: Redis = Depends(get_redis), settings=Depends(get_settings)) -> PlainTextResponse:

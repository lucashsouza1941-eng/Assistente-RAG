...
def create_app() -> FastAPI:
    ...
    @app.get('/health')
    async def health(settings=Depends(get_settings), db=Depends(get_db_session), redis=Depends(get_redis)):
        ...
        try:
            ...
            redis_ok = False
        try:
            vector_ok = bool((await db.execute(text("SELECT EXISTS(SELECT 1 FROM pg_extension WHERE extname = 'vector')"))).scalar())
        except Exception:
            vector_ok = False
        if not (db_ok and redis_ok and vector_ok):
            raise HTTPException(status_code=503, detail={'status': 'unhealthy', 'db': 'ok' if db_ok else 'down', 'redis': 'ok' if redis_ok else 'down', 'vector_store': 'ok' if vector_ok else 'down'})
